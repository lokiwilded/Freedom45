...
async function main() {
  ...
  console.log(`\n=== analyze_earnings_momentum for ${ticker} ===`);
  const result = await analyzeEarningsMomentum(ticker);

  assertType(result.summary, "string", "summary is a string");
  assert(result.summary.length > 0, "summary is non-empty");
  assertInArray(
    result.verdict,
    ["Strong", "Improving", "Stable", "Softening", "Weak", "No Data"],
    "verdict is valid"
  );
  assertType(result.score, "number", "score is a number");
  assert(result.score >= 0 && result.score <= 100, "score between 0 and 100");
  assertType(result.beatStreak, "number", "beatStreak is a number");
  assertType(result.missStreak, "number", "missStreak is a number");
